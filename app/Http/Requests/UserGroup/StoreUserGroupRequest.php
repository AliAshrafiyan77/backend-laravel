...
class StoreUserGroupRequest extends FormRequest
{
    ...
    {
        return auth()->check() && auth()->user()?->hasPermissionTo('store-user-group');
    }
    ...
    {
        return [
            'name' => ['required', 'min:3', 'max:225', 'string', 'unique:user_groups'],
            'display_name' => ['required', 'min:3', 'max:225', 'string', 'unique:user_groups'],
        ];
    }
